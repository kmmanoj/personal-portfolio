...
tags: [intermediate, sdn, cloud, networking]
...
In one of my previous blog articles, I presented a brief and elementary introduction about Software Defined Networks. In this blog article, we shall walk through setting up GNS3, and further implement a simple controller using RYU framework on a network of SDN switches in GNS3.
...
_NOTE: Before moving forward ensure that ryu is correctly installed and is working by running ryu-manager in a terminal with venv active._
...
_Ensure that the IP address used in the above command is in fact the IP address of one of the interfaces on your host machine where GNS3 server is installed._
...
_Think: Why does the link connecting NAT and Open vSwitch see ICMP packets between the two PCs?_
